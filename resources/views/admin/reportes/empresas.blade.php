                                        ...
                                        <div class="col">
                                            <div class="d-flex mt-2">
                                                ...
                                                <div class="flex-grow-1 ms-2 ps-1">
                                                    <h5 class="mb-1">
                                                        {{ ConfigSistema::moneyFormat($data['con_vacantes6m'],0) }}
                                                    </h5>
                                                    <p class="text-muted text-truncate mb-0">Con vacantes (6 meses)</p>
                                                </div>
                                            </div>
                                        </div>
    ...
    <div class="col-xl-4">
        <div class="card">
            <div class="card-body">
                <div class="d-flex align-items-start">
                    <div class="flex-grow-1">
                        <h5 class="card-title mb-2">Sector</h5>
                    </div>
                </div>

                <div id="chart-programa"  class="apex-charts" dir="ltr"></div>
                <div class="mt-1 px-2">
                    @foreach($data['empresas_pa']['labels'] as $k => $label)
                        <div class="order-wid-list d-flex justify-content-between border-bottom">
                            <p class="mb-0">
                                <i class="mdi mdi-square-rounded font-size-10 me-2" style="color:{{ $data['empresas_pa']['colores'][$k]  }}"></i>
                                {{ $label }}
                            </p>
                            <div>
                                <span class="pe-5">
                                    {{ ConfigSistema::moneyFormat($data['empresas_pa']['num'][$k],0) }}
                                </span>
                            </div>
                        </div>
                    @endforeach 
...
@section('script-bottom')
    <script type="text/javascript">
        ...
        var options = {
          ...
          series: [{
            name: 'Registrados',
            data: [ {!! implode(',',$data['empresas_registrados_g1']['num']) !!} ]
          }],
          colors: chartBarColors,
          // colors: ["#776acf", "#e4e1f5"],
          xaxis: {
            categories: [ {!! "'".implode("','",$data['empresas_registrados_g1']['cat'])."'" !!} ],
             title: {
        ...
        var options = {
          ...
          series: [ {!! implode(',',$data['empresas_pa']['num']) !!} ],
          labels: [ {!! "'".implode("','",$data['empresas_pa']['labels'])."'" !!} ],
          colors: [ {!! "'".implode("','",$data['empresas_pa']['colores'])."'" !!}  ],
          legend: {
            show: false
          }
        };
